...
export default function UpgradePage() {
  ...
  const handleCredentialsUpgrade = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!data.success) {
        ...
        // Refresh the session to reflect the upgrade
        await update();
        router.push('/');
        router.refresh();
      }
  ...
  const handleOAuthUpgrade = (provider: string) => {
    // OAuth will link to the existing account via the signIn callback
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">保存你的学习进度</h1>
          <p className="text-gray-600 mt-2">升级为正式账号，保留所有数据</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <>
              <div className="space-y-3">
                {hasGoogle && (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-gray-700">Google 绑定</span>
                  </button>
                )}

                {hasWeChat && (
                  <button
                    onClick={() => handleOAuthUpgrade('wechat')}
                    className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
                  >
                    ...
                    <span className="text-sm font-medium text-gray-700">微信绑定</span>
                  </button>
                )}
              </div>
              ...
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-white text-gray-500">或设置邮箱密码</span>
                </div>
              </div>
            </>
          )}
          ...
          {/* Email/Password Form */}
          <form onSubmit={handleCredentialsUpgrade} className="space-y-4">
            <div>
